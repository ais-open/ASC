...
namespace AzureServiceCatalog.Helpers
{
    public class ActivationHelper
    {
        ...
        public async Task SaveActivation(ActivationInfo activationInfo)
        {
            ...
            ADGroup orgGroup = await AzureADGraphApiUtil.CheckIfADGroupExistsByOrgName(activationInfo.Organization.Id, defaultAdGroup);

            if (orgGroup == null)
            {
                await AzureADGraphApiUtil.CreateGroup(activationInfo.Organization.Id, defaultAdGroup);
                orgGroup = await AzureADGraphApiUtil.CheckIfADGroupExistsByOrgName(activationInfo.Organization.Id, defaultAdGroup);
            }
            ...
            //var orgGroups = await AzureADGraphApiUtil.GetAllGroupsForOrganization(activationInfo.Organization.Id);
            ...
            if (isRunningInAzureGeneral)
            {
                ...
                notificationHelper.SendActivationNotification(activationInfo.Organization);
            }
        ...
        public async Task SaveEnrolledSubscriptions(SubscriptionsViewModel subscriptionsVM)
        {
            ...
            foreach (var subscription in subscriptionsVM.Subscriptions)
            {
                ...
                if (isSwitchingEnrollmentOn || isSwitchingHostingOn)
                {
                    ...
                    {
                        await AzureResourceManagerUtil.GrantRoleToServicePrincipalOnSubscriptionAsync(organization.ObjectIdOfCloudSenseServicePrincipal, subscription.Id);
                    });
                }
                ...
                if (isSwitchingEnrollmentOff || isSwitchingHostingOff)
                {
                    ...
                    await AzureResourceManagerUtil.RevokeRoleFromServicePrincipalOnSubscription(organization.ObjectIdOfCloudSenseServicePrincipal, subscription.Id);
                }
        ...
        private async Task SaveHostSubscription(ActivationInfo activationInfo)
        {
            ...
            {
                var organizations = await AzureResourceManagerUtil.GetUserOrganizations();
            ...
            Run.WithProgressBackOff(5, 1, 5, async() =>
            {
                await AzureResourceManagerUtil.GrantRoleToServicePrincipalOnSubscriptionAsync(activationInfo.HostSubscription.ServicePrincipalId, activationInfo.HostSubscription.SubscriptionId);
            });
        ...
        private async Task SaveEnrolledSubscriptions(ActivationInfo activationInfo)
        {
            foreach (var enrolledSubscription in activationInfo.EnrolledSubscriptions)
            {
                Run.WithProgressBackOff(5, 1, 5, async() =>
                {
                    await AzureResourceManagerUtil.GrantRoleToServicePrincipalOnSubscriptionAsync(activationInfo.HostSubscription.ServicePrincipalId, enrolledSubscription.SubscriptionId);
                });
